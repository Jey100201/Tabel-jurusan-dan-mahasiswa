...
<table class="table table-hover table-striped">
    ...
    <tbody>
        <?php 
        include "koneksi.php";
        $qry_histori=mysqli_query($conn, "select * from pinjam_buku order by id_pinjam_buku desc");
        $no=0;
        while($dt_histori=mysqli_fetch_array($qry_histori)) {
            $no++;
            //menampilkan buku yang dipinjam
            $buku_dipinjam="<ol>";
            $qry_buku=mysqli_query($conn, "select * from detail_pinjam_buku join buku on buku.id_buku=detail_pinjam_buku.id_buku where id_pinjam_buku = '".$dt_histori['id_pinjam_buku']."'");
            while ($dt_buku=mysqli_fetch_array($qry_buku)) {
                $buku_dipinjam.="<li>".$dt_buku['judul_buku']."</li>";
            }
            $buku_dipinjam.="</ol>";
            //menampilkan status sudah kembali atau belum
            $qry_cek_kembali=mysqli_query($conn,"select * from pengembalian_buku where id_pinjam_buku = '".$dt_histori['id_pinjam_buku']."'");
            if(mysqli_num_rows($qry_cek_kembali)>0) {
                ...
                $button_kembali="<a href='pengembalian_buku.php?id=".$dt_histori['id_pinjam_buku']."' class='btn btn-warning' onclick='return confirm(\"Yakin ingin mengembalikan?\")'>Kembalikan</a>";
            }
        ?>
            <tr>
